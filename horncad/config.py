...
ALLOWED_OSSE_SOLVES = {"s"}
ALLOWED_AREA_TARGETS = {"equivalent_round_mouth"}
ALLOWED_OUTPUT_SCOPES = {"quarter", "half", "full"}
ALLOWED_REFINEMENT_SOLVES = {"morph_rate", "n", "q"}
...
def validate_config(config: Mapping[str, Any]) -> None:
    ...
    for profile_name in ("horizontal", "vertical"):
        base = ["profiles", profile_name]
        _require_number(config, base + ["coverage"], errors, minimum=0.0, maximum=180.0)
        _require_number(config, base + ["k"], errors, minimum=0.0, maximum=10.0)
        _optional_number(config, base + ["s_initial"], errors, minimum=0.0, maximum=4.0)

    _require_enum(config, ["osse", "solve"], ALLOWED_OSSE_SOLVES, errors)
    _require_number(config, ["osse", "q"], errors, minimum=0.99, maximum=1.0)
    _require_number(config, ["osse", "n"], errors, minimum=2.0, maximum=10.0)
    _validate_s_bounds(config, errors)

    _require_number(config, ["morph", "start"], errors, minimum=0.0)
    _require_number(config, ["morph", "rate"], errors, minimum=0.0, exclusive_minimum=True)
    _require_enum(config, ["morph", "area_target"], ALLOWED_AREA_TARGETS, errors)

    _validate_refinement_solve(config, errors)
    _validate_number_bounds(
        config,
        ["refinement", "morph_rate_bounds"],
        errors,
        minimum=0.0,
        maximum=8.0,
        exclusive_minimum=True,
    )
    _validate_number_bounds(config, ["refinement", "n_bounds"], errors, minimum=2.0, maximum=10.0)
    _validate_number_bounds(config, ["refinement", "q_bounds"], errors, minimum=0.99, maximum=1.0)
...
def _can_derive_mouth_dimension(config: Mapping[str, Any]) -> bool:
    required_paths = [
        ...
        ["throat", "conic_extension", "exit_angle"],
        ["length", "max"],
        ["profiles", "horizontal", "coverage"],
        ["profiles", "horizontal", "k"],
        ["profiles", "vertical", "coverage"],
        ["profiles", "vertical", "k"],
        ["osse", "q"],
        ["osse", "n"],
    ]
...
def _solve_s_for_axis(config: Mapping[str, Any], axis: str, target_boundary_distance: float) -> float:
    ...
    if profile_length <= 0.0:
        return float(config["osse"]["s_bounds"][0])
...
def _validate_s_bounds(config: Mapping[str, Any], errors: List[str]) -> None:
    value = _get(config, ["osse", "s_bounds"])
    if not isinstance(value, list) or len(value) != 2 or not all(_is_number(item) for item in value):
        errors.append("osse.s_bounds must be a two-number list")
        return
    lower, upper = float(value[0]), float(value[1])
    if lower < 0.0 or upper > 4.0:
        errors.append("osse.s_bounds values must be within 0..4")
    if lower > upper:
        errors.append("osse.s_bounds lower value must be less than or equal to upper value")
...
def _validate_refinement_solve(config: Mapping[str, Any], errors: List[str]) -> None:
    value = _get(config, ["refinement", "solve"])
    if not isinstance(value, list):
        errors.append("refinement.solve must be a list")
        return
    for item in value:
        if item not in ALLOWED_REFINEMENT_SOLVES:
            errors.append(f"refinement.solve entries must be one of {sorted(ALLOWED_REFINEMENT_SOLVES)}")
            return
