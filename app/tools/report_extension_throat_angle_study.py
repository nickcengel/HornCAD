...
def render_index(root: Path, manifest: dict[str, Any],
    ...
    for row in sorted(candidates, key=lambda item: (
        ...
        candidate_rows.append(
            ...
            f"<td data-sort='{_sort(row.get('osse_length_mm'))}'>{_number(row.get('osse_length_mm'),1)} mm</td>"
            f"<td data-column='s' data-sort='{_sort(row.get('derived_s'))}'>{_number(row.get('derived_s'),3)}</td>"
            f"<td data-column='s' class='{'delta-positive' if s_change is not None and s_change > 0 else 'delta-negative' if s_change is not None and s_change < 0 else ''}' "
            f"data-sort='{_sort(s_change)}' title='Parent S: {_number(parent_s,3)}'>"
            f"{_signed_percent(s_change)}</td>"
