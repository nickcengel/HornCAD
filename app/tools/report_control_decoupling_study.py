...
def _active_searches(runtime: dict[str, Any]) -> set[str]:
    """Reconstruct scheduler-active paths even before search state exists."""
    active: set[str] = set()
    for event in runtime.get("events", []):
        path = event.get("search")
        if not isinstance(path, str):
            continue
        if event.get("status") == "started":
            active.add(path)
        elif event.get("status") in {"complete", "failed"}:
            active.discard(path)
    return active if runtime.get("status") == "running" else set()
...
def build_progress(root: Path, manifest: dict[str, Any],
    ...
    benchmark_registry = _read_json(root / "benchmarks.json")
    skipped = {item["search"] for item in runtime.get("skipped_searches", [])}
    active = _active_searches(runtime)
    ...
    for wave in plan.get("wave_counts", {}):
        ...
        for item in searches:
            ...
            if item["path"] in skipped:
                ...
                if status == "not-started" and item["path"] in active:
                    status = "running"
...
def render_index(manifest: dict[str, Any], progress: dict[str, Any]) -> str:
    ...
    benchmarks = progress.get("benchmarks", [])
    ...
    for item in candidates:
        ...
        candidate_rows.append(
            ...
            f"<td data-sort='{item['coverage_deg']}'>{item['coverage_deg']}°</td>"
            f"<td data-sort='{item['mouth_mm']}'>{item['mouth_mm']} mm</td>"
    ...
    for item in progress["searches"]:
        ...
        search_rows.append(
            ...
            f"<td>{html.escape(item['wave'])}</td>"
            f"<td data-sort='{item['coverage_deg']}'>{item['coverage_deg']}°</td>"
            f"<td data-sort='{item['mouth_mm']}'>{item['mouth_mm']} mm</td>"
            f"<td data-sort='{html.escape(item['status'])}'><span class='badge {status_class}'>{html.escape(item['status'])}</span></td>"
            f"<td data-sort='{item['date_unix']}'>{_date(item['date_unix'])}</td>"
    ...
    for item in benchmarks:
        link = (f"<a href='{html.escape(item['report'])}'>{html.escape(item['id'])}</a>"
                if item.get("report") else html.escape(item["id"]))
        benchmark_rows.append(
            f"<tr><td>{link}</td><td data-sort='{item['score']}'>{_number(item['score'])}</td>"
            f"<td data-sort='{item['coverage_deg']}'>{item['coverage_deg']}°</td>"
            f"<td data-sort='{item['mouth_mm']}'>{item['mouth_mm']} mm</td>"
            f"<td data-sort='{item['length_mm']}'>{item['length_mm']:.1f}</td>"
            f"<td data-sort='{_sort(item.get('s'))}'>{_number(item.get('s'),2)}</td>"
            f"<td data-sort='{item['k']}'>{item['k']:g}</td>"
            f"<td data-sort='{item['n']}'>{item['n']:g}</td>"
            f"<td>{html.escape(item['search'])}</td></tr>")
    grid = []
    ...
    plot_points = [{
        **{key: row.get(key) for key in (
            "id", "coverage_deg", "mouth_mm", "length_factor", "k", "n", "s")},
        "status": progress["coordinate_status"].get(row["id"], row["status"]),
    } for row in manifest["coordinates"]
        if isinstance(row.get("s"), (int, float))]
    plot_points.extend({
        "id": item["id"], "coverage_deg": item["coverage_deg"],
        "mouth_mm": item["mouth_mm"], "length_factor": item["length_factor"],
        "k": item["k"], "n": item["n"], "s": item.get("s"),
        "score": item["score"], "status": "benchmark",
    } for item in benchmarks)
...
<section><h2>Sampling extent</h2><p>Select a cell to inspect the registered L-factor/K/N cloud.</p><table class='sampling-matrix'><thead><tr><th>Cell</th>{''.join(f'<th>{a}°</th>' for a in angles)}</tr></thead><tbody>{''.join(matrix_buttons)}</tbody></table><h3 id='sampling-selection'></h3><div class='plot-legend'><span class='legend-item'><i class='legend-dot' style='background:#69d6c8'></i>Planned</span><span class='legend-item'><i class='legend-dot' style='background:#f0a43c'></i>Running</span><span class='legend-item'><i class='legend-dot' style='background:#58d68d'></i>Newly complete</span><span class='legend-item'><i class='legend-dot' style='background:#f6d39a'></i>Reused historical</span><span class='legend-item'><i class='legend-dot' style='background:#b388ff'></i>Prior best benchmark ◆</span><span class='legend-item'><i class='legend-dot' style='background:#e57373'></i>Failed / rejected</span></div><canvas id='sampling-3d' class='sampling-canvas'></canvas><p class='muted'>Drag to rotate. Click the plot to enable wheel zoom; click elsewhere to release it. Axes are length factor, K, and N—S is not treated as an independent coordinate.</p></section>
<section><h2>Historical optimized benchmarks</h2><p class='muted'>One highest-scoring compatible prior result per cell, rescored with the current diagnostics. These points are external comparisons and never enter the canonical fit, closure, pruning, or validation.</p><table class='sortable-table'><thead><tr><th data-sort='text'>Benchmark</th><th data-sort='number'>Surface score</th><th data-sort='number'>Coverage</th><th data-sort='number'>Mouth</th><th data-sort='number'>Length mm</th><th data-sort='number'>S</th><th data-sort='number'>K</th><th data-sort='number'>N</th><th data-sort='text'>Source search</th></tr></thead><tbody>{''.join(benchmark_rows)}</tbody></table></section>
...
<script>(()=>{{const points={json.dumps(plot_points,separators=(',',':'))};let key='350:40',yaw=-.65,pitch=.65,zoom=.82,drag=null,wheel=false;const canvas=document.getElementById('sampling-3d'),label=document.getElementById('sampling-selection');const cell=()=>{{const [m,a]=key.split(':').map(Number);return points.filter(p=>p.mouth_mm===m&&p.coverage_deg===a&&Number.isFinite(p.length_factor));}};const pointColor=status=>{{if(status==='benchmark')return'#b388ff';if(status==='reused')return'#f6d39a';if(status==='complete')return'#58d68d';if(status==='running')return'#f0a43c';if(['failed','error','blocked','geometry-rejected','geometry-redundant'].includes(status))return'#e57373';return'#69d6c8';}};function draw(){{const ps=cell();if(!ps.length)return;const ratio=devicePixelRatio||1,r=canvas.getBoundingClientRect();canvas.width=Math.round(r.width*ratio);canvas.height=Math.round(r.height*ratio);const c=canvas.getContext('2d');c.setTransform(ratio,0,0,ratio,0,0);c.clearRect(0,0,r.width,r.height);const ranges=['length_factor','k','n'].map(name=>{{const v=ps.map(p=>p[name]),lo=Math.min(...v),hi=Math.max(...v),pad=Math.max((hi-lo)*.1,.08);return[lo-pad,hi+pad];}});const norm=p=>({{x:(p.length_factor-ranges[0][0])/(ranges[0][1]-ranges[0][0])*2-1,y:(p.k-ranges[1][0])/(ranges[1][1]-ranges[1][0])*2-1,z:(p.n-ranges[2][0])/(ranges[2][1]-ranges[2][0])*2-1,p}});const scale=Math.min(r.width,r.height)*.3*zoom;const project=v=>{{const x=v.x*Math.cos(yaw)-v.y*Math.sin(yaw),y=v.x*Math.sin(yaw)+v.y*Math.cos(yaw);return{{x:r.width/2+x*scale,y:r.height/2+(y*Math.cos(pitch)-v.z*Math.sin(pitch))*scale,d:y*Math.sin(pitch)+v.z*Math.cos(pitch),p:v.p}};}};const origin=project({{x:-1,y:-1,z:-1}});function axis(end,text){{end=project(end);c.strokeStyle='#536774';c.beginPath();c.moveTo(origin.x,origin.y);c.lineTo(end.x,end.y);c.stroke();c.fillStyle='#d7e2e8';c.font='12px system-ui';c.fillText(text,end.x+5,end.y-5);}}axis({{x:1,y:-1,z:-1}},'Length factor');axis({{x:-1,y:1,z:-1}},'K');axis({{x:-1,y:-1,z:1}},'N');ps.map(norm).map(project).sort((a,b)=>a.d-b.d).forEach(v=>{{c.beginPath();if(v.p.status==='benchmark'){{c.moveTo(v.x,v.y-7);c.lineTo(v.x+7,v.y);c.lineTo(v.x,v.y+7);c.lineTo(v.x-7,v.y);c.closePath();}}else c.arc(v.x,v.y,4,0,Math.PI*2);c.fillStyle=pointColor(v.p.status);c.fill();c.strokeStyle='#071015';c.stroke();}});}}function render(){{const [m,a]=key.split(':');label.textContent=`${{m}} mm / ${{a}}° — ${{cell().length}} registered and benchmark coordinates`;document.querySelectorAll('[data-sampling-key]').forEach(b=>b.classList.toggle('active',b.dataset.samplingKey===key));draw();}}document.querySelectorAll('[data-sampling-key]').forEach(b=>b.onclick=()=>{{key=b.dataset.samplingKey;render();}});canvas.onpointerdown=e=>{{drag=[e.clientX,e.clientY];canvas.setPointerCapture(e.pointerId);}};canvas.onpointermove=e=>{{if(!drag)return;yaw+=(e.clientX-drag[0])*.008;pitch=Math.max(.15,Math.min(1.3,pitch+(e.clientY-drag[1])*.006));drag=[e.clientX,e.clientY];draw();}};canvas.onpointerup=()=>drag=null;canvas.onclick=()=>{{wheel=true;canvas.classList.add('wheel-active');}};document.addEventListener('pointerdown',e=>{{if(!canvas.contains(e.target)){{wheel=false;canvas.classList.remove('wheel-active');}}}});canvas.addEventListener('wheel',e=>{{if(!wheel)return;e.preventDefault();zoom=Math.max(.55,Math.min(1.4,zoom-e.deltaY*.001));draw();}},{{passive:false}});addEventListener('resize',draw);
document.querySelectorAll('[data-column-toggle]').forEach(b=>b.onclick=()=>{{const on=b.getAttribute('aria-pressed')!=='true';b.setAttribute('aria-pressed',String(on));document.querySelectorAll(`[data-column="${{b.dataset.columnToggle}}"]`).forEach(x=>x.hidden=!on);}});const ct=document.getElementById('candidate-table'),more=document.getElementById('candidate-show-more'),cc=document.getElementById('candidate-count');let angle='all',limit=25;function candidates(){{let match=0,shown=0;Array.from(ct.tBodies[0].rows).forEach(r=>{{const ok=angle==='all'||r.dataset.coverageAngle===angle;if(ok)match++;r.hidden=!(ok&&shown++<limit);}});shown=Math.min(match,limit);cc.textContent=`Showing ${{shown}} of ${{match}}`;more.hidden=shown>=match;}}document.querySelectorAll('[data-angle-filter]').forEach(b=>b.onclick=()=>{{angle=b.dataset.angleFilter;limit=25;document.querySelectorAll('[data-angle-filter]').forEach(x=>x.setAttribute('aria-pressed',String(x===b)));candidates();}});more.onclick=()=>{{limit+=25;candidates();}};const st=document.getElementById('subsearch-table'),sc=document.getElementById('subsearch-count');let subAngle='all';function subsearches(a=subAngle){{subAngle=a;let n=0;Array.from(st.tBodies[0].rows).forEach(r=>{{r.hidden=!(a==='all'||r.dataset.subsearchCoverageAngle===a);if(!r.hidden)n++;}});sc.textContent=`${{n}} sub-searches`;}}document.querySelectorAll('[data-subsearch-angle-filter]').forEach(b=>b.onclick=()=>{{document.querySelectorAll('[data-subsearch-angle-filter]').forEach(x=>x.setAttribute('aria-pressed',String(x===b)));subsearches(b.dataset.subsearchAngleFilter);}});document.querySelectorAll('table.sortable-table').forEach(table=>{{const hs=Array.from(table.querySelectorAll('th[data-sort]'));let active=-1,dir='desc';hs.forEach(h=>{{h.classList.add('sortable');h.tabIndex=0;const sort=()=>{{const column=h.cellIndex;dir=active===column&&dir==='desc'?'asc':'desc';active=column;hs.forEach(x=>x.removeAttribute('aria-sort'));h.setAttribute('aria-sort',dir==='asc'?'ascending':'descending');const mul=dir==='asc'?1:-1,rows=Array.from(table.tBodies[0].rows);rows.sort((a,b)=>{{let x=a.cells[column]?.dataset.sort??a.cells[column]?.textContent??'',y=b.cells[column]?.dataset.sort??b.cells[column]?.textContent??'';if(h.dataset.sort==='number'){{x=Number(x);y=Number(y);x=Number.isFinite(x)?x:-Infinity;y=Number.isFinite(y)?y:-Infinity;}}return(x<y?-1:x>y?1:0)*mul;}});table.tBodies[0].replaceChildren(...rows);if(table===ct)candidates();if(table===st)subsearches();}};h.onclick=sort;h.onkeydown=e=>{{if(e.key==='Enter'||e.key===' '){{e.preventDefault();sort();}}}};}});}});render();candidates();subsearches();}})();</script></main></body></html>"""
