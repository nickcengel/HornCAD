    ...
    for mouth in manifest["domain"]["mouth_mm"]:
        ...
        for angle in manifest["domain"]["coverage_deg"]:
            ...
            cells.append(
                ...
                f"<span>{sum(row['status']=='planned' for row in cell)} BEM planned · "
...
<section><h2>Project range</h2><table><tr><th>Coverage half-angles</th><th>Mouths</th><th>Length factors</th><th>K</th><th>N</th><th>Registered / new BEM</th></tr><tr><td>{', '.join(map(str,angles))}°</td><td>{', '.join(map(str,manifest['domain']['mouth_mm']))} mm</td><td>0.80 / 1.00 / 1.20<br>0.70 / 1.30 sentinels</td><td>2.5 / 4 / 5.5</td><td>4 / 8 / 16<br>K4/N10 reference</td><td>800 / {counts.get('planned',0)}</td></tr></table><p class='muted'>Manifest SHA-256: <code>{progress['manifest_sha256']}</code></p></section>
