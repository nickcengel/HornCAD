...
- the mouth return is closed with a dedicated spherical/cylindrical cap at
  `mouth_sag_radius - mouth_rear_offset`, bounded by the inner mouth-offset edge
  and the horn wall projected onto that same return surface; do not treat the
  rear return ring as another radial minimum-wall station,
...
- Body cap rings must reuse the existing boundary rings at the rear mouth and
  ...
  repaired.
- Intermediate mouth cap rings are generated by interpolating physical `x/y`
  between the mouth-offset edge and horn-wall intersection, then solving `z` on
  the return sphere/cylinder. Do not use vector/geodesic interpolation there; it
  can bow through the acoustic opening near corners.
- Intermediate mouth cap rings also get a small zero-at-boundary axial setback to
  avoid near-coplanar corner poke-through. The boundary curves themselves remain
  unchanged.
