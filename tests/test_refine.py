...
def test_refine_project_searches_candidates_and_improves_area_fit():
    ...
    assert result.search_space.effective_ranges["q"][0] >= config["refinement"]["q_bounds"][0]
    assert result.search_space.effective_ranges["q"][1] <= config["refinement"]["q_bounds"][1]
    ...
    assert result.best.config["profiles"]["horizontal"]["k"] == config["profiles"]["horizontal"]["k"]
    assert result.best.config["profiles"]["vertical"]["k"] == config["profiles"]["vertical"]["k"]
    assert 0.25 <= result.best.config["morph"]["rate"] <= 8.0
    assert 2.0 <= result.best.config["osse"]["n"] <= 10.0
    assert 0.99 <= result.best.config["osse"]["q"] <= 1.0
...
def test_generate_refinement_review_artifacts(tmp_path):
    ...
    assert "Dependent solve: S(p), recomputed for every candidate" in report
    assert "Candidate target: mean H/V circular OS-SE reference" in report
    ...
    assert refined["profiles"]["horizontal"]["k"] == 1.0
...
def test_build_search_space_scales_with_design_and_global_bounds():
    ...
    assert search_space.effective_ranges["morph_rate"][0] == config["refinement"]["morph_rate_bounds"][0]
    assert search_space.effective_ranges["morph_rate"][1] == config["refinement"]["morph_rate_bounds"][1]
    assert search_space.effective_ranges["q"] == tuple(config["refinement"]["q_bounds"])
