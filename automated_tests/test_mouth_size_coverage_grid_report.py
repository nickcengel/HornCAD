...
class MouthSizeCoverageGridReportTests(unittest.TestCase):
    ...
    def test_candidate_columns_are_toggleable_and_details_start_hidden(self) -> None:
        ...
        self.assertIn("id='design-surface'", report)
        self.assertIn("const surfacePoints = [", report)
        self.assertIn("Drag to rotate · wheel to zoom", report)
        self.assertIn("raw.some(point => !point)", report)
